...
async function loadNLStatData(geojsonFeatures, onProgress) {
  ...
  try {
    ...
    if (cached) {
      ...
      if (Date.now() - ts < CACHE_TTL) {
        onProgress?.('Using cached CBS data…');
        return data;
      }
    }
  } catch(_) {}

  // 2. Try pre-built static dataset (generated by build/fetch_nl_data.py)
  // This covers all 352 gemeenten and does not depend on the CBS API.
  try {
    onProgress?.('Loading CBS data (nl_data.json)…');
    const r = await fetch('./data/nl_data.json', { signal: AbortSignal.timeout(10000) });
    if (r.ok) {
      const data = await r.json();
      if (data.Gemeente && Object.keys(data.Gemeente).length > 100) {
        onProgress?.('CBS data ready.');
  ...
  // 3. Live CBS OData4 (may be unavailable — redirects to HTML as of 2026)
  onProgress?.('Fetching CBS data…');

  try {
    onProgress?.('Loading CBS metadata (85039NED)…');
    const props  = await fetchDataProperties();
    const dynMap = buildVariableMapFromProps(props);
    const varMap = { ...CBS_KNOWN_VARS, ...dynMap };
    const varKeys = Object.keys(varMap);

    onProgress?.('Fetching gemeente observations from CBS (may take ~5s)…');
    const rows = await fetchObservations(varKeys);

    if (!rows.length) throw new Error('CBS returned 0 rows');

    onProgress?.('Processing CBS data…');
    const gemeenteMap = reshapeRows(rows, varMap);
    if (geojsonFeatures) injectProvinceNames(gemeenteMap, geojsonFeatures);
    const nlTotals    = computeNLTotals(gemeenteMap);
    const result      = { Gemeente: gemeenteMap, 'Netherlands Total': nlTotals };
    ...
    console.warn('CBS API unavailable, falling back to demo data:', err.message);
    onProgress?.('CBS API unavailable — loading demo data…');
    return buildDemoData(geojsonFeatures);
  }
}
